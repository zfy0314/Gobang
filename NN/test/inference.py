...
def inference(input_tensor, train, regularizer):
    with tf.variable_scope('layer1-conv1'):
        conv1_weights = tf.get_variable("weights", [CONV1_SIZE, CONV1_SIZE, NUM_CHANNALS, CONV1_DEEP],
    ...
    with tf.variable_scope('layer5-fc1'):
        ...
        fc1_biases = tf.get_variable("biases", [FC_SIZE],
                                     initializer = tf.truncated_normal_initializer(stddev = 0.1))
        fc1 = tf.nn.relu(tf.matmul(reshaped, fc1_weigths) + fc1_biases)
        if train: fc1 = tf.nn.dropout(fc1, 0.5)

    with tf.variable_scope('layer6-fc2'):
        fc2_weigths = tf.get_variable("weights", [FC_SIZE, NUM_LABELS],
                                      initializer = tf.truncated_normal_initializer(stddev = 0.1))
        if not None == regularizer:
            tf.add_to_colletion('losses', regularizer(f2_weights))
        fc2_biases = tf.get_variable("biases", [NUM_LABEL],
                                     initializer = tf.truncated_normal_initializer(stddev = 0.1))
